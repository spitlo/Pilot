...
export default function ChannelInterface(pilot, id, node) {
  ...
  this.setEnv = function (data) {
    if (this.lastEnv && performance.now() - this.lastEnv < 100) {
      return
    }
    ...
    if (!isNaN(data.attack)) {
      this.node.envelope.attack = clamp(data.attack, 0.01, 1.0)
    }
    if (!isNaN(data.decay)) {
      this.node.envelope.decay = clamp(data.decay, 0.01, 1.0)
    }
    if (!isNaN(data.sustain)) {
      this.node.envelope.sustain = clamp(data.sustain, 0.01, 1.0)
    }
    if (!isNaN(data.release)) {
      this.node.envelope.release = clamp(data.release, 0.01, 1.0)
    }
  ...
  this.setOsc = function (data) {
    if (this.lastOsc && performance.now() - this.lastOsc < 100) {
      return
    }
  ...
  this.updateEnv = function (data, force = false) {
    if (pilot.animate !== true) {
      return
    }
    if (force !== true && (!data || !data.isEnv)) {
      return
    }
    if (!this.node.envelope) {
      return
    ...
    setContent(
      this.env_el,
      `${to16(this.node.envelope.attack)}${to16(
        this.node.envelope.decay
      )}${to16(this.node.envelope.sustain)}${to16(this.node.envelope.release)}`
    )
  }
    ...
    setContent(
      ...
      `${
        this.node.oscillator
          ? wavCode(this.node.oscillator._oscillator.type)
          : '--'
      }${
        this.node.modulation
          ? wavCode(this.node.modulation._oscillator.type)
          : '--'
      }`
    )
  }
  ...
  function parseOsc(msg) {
    if (msg.length < 2) {
      console.warn(`Misformatted osc`)
      return
    }
    return {
      isOsc: true,
      wav: msg.length > 1 ? wavName(msg.substr(0, 2)) : null,
      mod: msg.length > 3 ? wavName(msg.substr(2, 2)) : null,
      string: 'osc',
    }
  }
